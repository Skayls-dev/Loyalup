-- Add free-amount service label column to transactions
alter table public.transactions
  add column if not exists service_nom_libre text null;

-- Update the credit_points_transaction RPC to accept and persist the free label
create or replace function public.credit_points_transaction(
  p_provider_user_id uuid,
  p_pending_transaction_id uuid,
  p_montant numeric,
  p_service_id uuid default null,
  p_service_nom_libre text default null
)
returns table (
  success boolean,
...
revoke all on function public.credit_points_transaction(uuid, uuid, numeric, uuid, text) from public;
grant execute on function public.credit_points_transaction(uuid, uuid, numeric, uuid, text) to service_role;
